...
type hoare : (pre:cond) -> (p:stmt) -> (post:cond) -> Type0 =
  | H_Skip :
    pre:cond ->
    hoare pre Skip pre // {P} Skip {P}

  | H_Assign :
    #x:var -> #e:expr ->
    #pre:cond ->
    hoare (fun s -> pre (override s x (eval_expr s e))) (Assign x e) pre // {pre /\ x = e} x := e {pre}
    // CONSULTA: esto se interpreta "si P vale con x = e, tenés permiso de conservar P luego de x := e"?

  | H_Seq :
    #p:stmt -> #q:stmt ->
    #pre:cond -> #mid:cond -> #post:cond ->
    hoare pre p mid ->       // {pre} p {mid} /\
    hoare mid q post ->      // {mid} q {post} ==>
    hoare pre (Seq p q) post // {pre} p;q {post}
...
let test1 : hoare (fun _ -> true) (Assign "x" (Const 1)) (fun s -> s "x" = 1) =
  H_Assign // gotta love implicit arguments!
